...
# based on https://github.com/mseitzer/pytorch-fid/blob/master/fid_score.py
    ...
    fake = np.zeros((512, 1000)) 
    real = np.zeros((512, 1000)) 
    
    for i, x in enumerate(sample_feature_iterator):
            
        fake[:,i] = next(sample_feature_iterator)
 
        real[:,i] = next(testset_feature_iterator)
    ...
    if not np.isfinite(covmean).all(): # avoid nan
        offset = np.eye(sigma_p.shape[0]) * 1e-11
        covmean = linalg.sqrtm((sigma_p + offset).dot(sigma_q + offset))

    covmean = covmean.real
        
    return np.dot(mu_p-mu_q,mu_p-mu_q) + np.trace(sigma_p + sigma_q - 2*covmean)
